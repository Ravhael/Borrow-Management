...
export const generateSubmitRejectCompanyEmail = (loan: any, _extra: any[] = [], approverLabel = 'Company', isUpdate = false, approvalInfo?: ApprovalInfo) => {
  if (!loan) {
...
export const generateSubmitRejectEntitasEmail = (loan: any, _extra: any[] = [], isUpdate = false, approvalInfo?: ApprovalInfo) => {
  if (!loan) {
    return `
...
export const generateSubmitRejectBorrowerEmail = (loan: any, _extra: any[] = [], isUpdate = false, approvalInfo?: ApprovalInfo) => {
  if (!loan) {
    return `
      <div style="padding:16px;font-family:Arial, sans-serif;color:#b71c1c;background:#fff3f3;border-radius:6px;border:1px solid rgba(183,28,28,0.08)">No persisted loan found — preview requires a loan stored in the DB.</div>`
  }
