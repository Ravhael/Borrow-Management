...
const extractReturnRejectionDecision = (loan: any, approvalInfo?: ApprovalInfo, fallbackApprover?: string): ReturnRejectionDecision => {
  if (!loan) return {}

  const returnRequests = Array.isArray(loan?.returnRequest)
    ? [...loan.returnRequest].reverse()
    : []
  const rejectionRequest = returnRequests.find((entry: any) => {
    const status = String(entry?.status || '').toLowerCase()
    return status.includes('reject')
  })

  const returnStatus = loan?.returnStatus || {}
  const warehouseStatus = loan?.warehouseStatus || {}

  const rejectedBy = rejectionRequest?.processedBy
    || returnStatus?.processedBy
    || warehouseStatus?.processedBy
    || warehouseStatus?.returnedBy
    || approvalInfo?.approverName
    || fallbackApprover
  const rejectedAt = rejectionRequest?.processedAt
    || returnStatus?.processedAt
    || warehouseStatus?.processedAt
    || warehouseStatus?.returnedAt
    || approvalInfo?.approvedAt
    || loan?.updatedAt
    || loan?.submittedAt
  const rejectionReason = rejectionRequest?.rejectionReason
    || returnStatus?.rejectionReason
    || warehouseStatus?.rejectionReason
  const rejectionNote = rejectionRequest?.processedNote
    || returnStatus?.note
    || warehouseStatus?.note
    || approvalInfo?.note
...
export const generateReturnRejectCompanyEmail = (loan: any, _extra: any[] = [], approverLabel = 'Admin', isUpdate = false, approvalInfo?: ApprovalInfo) => {
  if (!loan) {
  ...
  const fallbackApprover = approverLabel && String(approverLabel).trim() ? String(approverLabel).trim() : 'Warehouse Team'
  const rejectionDecision = extractReturnRejectionDecision(loan, approvalInfo, fallbackApprover)
  ...
  const headerHtml = shouldRenderApproved
    ...
    : `
      <div style="background:linear-gradient(120deg,#fff7f7,#fff);border-radius:10px;border:1.5px solid rgba(183,28,28,0.18);padding:16px;margin-bottom:18px;color:#5b0b0b;">
        <div style="font-size:15px;font-weight:800;margin-bottom:6px;color:#7d0f0f;">Pengajuan Pengembalian Barang Ditolak</div>
        <div style="font-size:13px;line-height:1.45;color:#5b0b0b;opacity:0.92;margin-bottom:12px;">
          Pengajuan pengembalian barang ini ditolak oleh tim Warehouse. Mohon tinjau detail berikut untuk tindak lanjut dan koordinasi lanjutan.
        </div>
...
export const generateReturnRejectEntitasEmail = (loan: any, _extra: any[] = [], isUpdate = false, approvalInfo?: ApprovalInfo) => {
  if (!loan) {
    return `
  ...
  const rejectionDecision = extractReturnRejectionDecision(loan, approvalInfo, 'Tim Warehouse')
  const rejectedByLabel = rejectionDecision.rejectedBy && String(rejectionDecision.rejectedBy).trim()
    ? String(rejectionDecision.rejectedBy).trim()
    : 'Tim Warehouse'
  const rejectedAtLabel = formatDateTimeDisplay(rejectionDecision.rejectedAt || loan?.updatedAt || loan?.submittedAt)
  ...
  const headerHtml = shouldRenderApproved
    ...
    : `
      <div style="background:linear-gradient(120deg,#fff7f7,#fff);border-radius:10px;border:1.5px solid rgba(183,28,28,0.18);padding:16px;margin-bottom:18px;color:#5b0b0b;">
        <div style="font-size:15px;font-weight:800;margin-bottom:6px;color:#7d0f0f;">Pengajuan Pengembalian Barang Ditolak</div>
        <div style="font-size:13px;line-height:1.45;color:#5b0b0b;opacity:0.92;margin-bottom:12px;">
          Notifikasi ini dikirim ke entitas peminjam untuk mencatat penolakan pengembalian barang. Silakan koordinasikan dengan peminjam atau tim Warehouse bila diperlukan.
        </div>
        <div style="background:#fff;border-radius:10px;border:1px dashed rgba(183,28,28,0.3);padding:12px 14px;">
          <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="border-collapse:collapse;font-size:13px;color:#4a0f0f;">
            <tr>
...
export const generateReturnRejectBorrowerEmail = (loan: any, _extra: any[] = [], isUpdate = false, approvalInfo?: ApprovalInfo) => {
  if (!loan) {
    return `
      <div style="padding:16px;font-family:Arial, sans-serif;color:#b71c1c;background:#fff3f3;border-radius:6px;border:1px solid rgba(183,28,28,0.08)">No persisted loan found — preview requires a loan stored in the DB.</div>`
  }
  ...
  const headerHtml = shouldRenderApproved
    ...
    : `
      <div style="background:#fff7f7;border-radius:10px;border:1.5px solid rgba(183,28,28,0.18);padding:16px;margin-bottom:18px;color:#5b0b0b;">
        ...
        <div style="font-weight:800;margin-bottom:6px;color:#7d0f0f;">Pengajuan Pengembalian Barang Anda Ditolak</div>
        <div style="font-size:13px;color:#5b0b0b;line-height:1.4;margin-bottom:12px;">
          Tim Warehouse belum dapat menyetujui pengembalian barang yang Anda ajukan. Mohon pelajari detail berikut dan lakukan perbaikan sebelum mengajukan kembali.
        </div>
        <div style="background:#fff;border-radius:10px;border:1px dashed rgba(183,28,28,0.3);padding:12px 14px;">
          <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="border-collapse:collapse;font-size:13px;color:#4a0f0f;">
            <tr>
